...
def main():
    ...
    model = build_unetplusplus_model(
    ...
    # criterion = TverskyLoss(alpha=0.5, beta=0.5)  # alpha=0.6, beta=0.4 penalizes FN more
    ...
    ckpt_path = config.MODEL_SAVE_DIR / config.CHECKPOINT_UNETPP

    best_val_iou = 0.0
    grad_accum_steps = config.GRAD_ACCUM_STEPS_UNETPP
    print(f"Starting U-Net++ training (batch_size={bs}, grad_accum={grad_accum_steps}, effective_batch={bs * grad_accum_steps})...")
    for epoch in range(1, config.EPOCHS + 1):
        ...
        print(
            f"U-Net++ Epoch {epoch}: Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | "
